...
// Fan and camera variables
float fanPosX = 0.0f, fanPosY = 1.6f, fanPosZ = 0.0f; // Fan hub position
float bladeAngle = 0.0f; // Blade rotation
float camAngle = 0.0f; // Camera rotation
float camDistance = 5.0f; // View distance
float camHeight = 1.6f;

// Lighting variables
const GLfloat light0_ambient[] = {0.2f, 0.2f, 0.2f, 1.0f}; // Key light
...
const GLfloat light1_ambient[] = {0.1f, 0.1f, 0.1f, 1.0f}; // Fill light
...
const GLfloat light2_ambient[] = {0.05f, 0.05f, 0.05f, 1.0f}; // Rim light
...
const GLfloat fan_ambient[] = {0.9f, 0.9f, 0.9f, 1.0f}; // White plastic
...
const GLfloat stand_ambient[] = {0.0f, 0.0f, 0.8f, 1.0f}; // Blue stand
...
void processInput(GLFWwindow* window, float deltaTime) {
    ...
    const float rotateSpeed = 1.0f; // Radians per second

    // Fan movement with WASD
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
        fanPosZ -= moveSpeed * deltaTime; // Forward
    }
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
        fanPosZ += moveSpeed * deltaTime; // Backward
    }
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
        fanPosX -= moveSpeed * deltaTime; // Left
    }
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
        fanPosX += moveSpeed * deltaTime; // Right
    }
